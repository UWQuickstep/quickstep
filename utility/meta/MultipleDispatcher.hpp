...
#ifndef QUICKSTEP_UTILITY_META_MULTIPLE_DISPATCHER_HPP_
#define QUICKSTEP_UTILITY_META_MULTIPLE_DISPATCHER_HPP_
...
/**
 * @brief Composable dispatcher that wires runtime values to compile-time types.
 */
...
using BoolDispatcher = MultipleDispatcher<Sequence<bool, true, false>>;

// -----------------------------------------------------------------------------
// Implementation of MultipleDispatcher.
...
struct MultipleDispatcher_0<
  ...
  static inline auto InvokeOn(const ArgTypes &...args,
    ...
    const auto failure = []() -> decltype(functor(TypeList<>())) {
      LOG(FATAL) << "Unexpected value for InvokeOn()";
    };
...
template <typename Dispatcher, std::size_t k, typename Enable = void>
struct DuplicateDispatcher;

template <typename Dispatcher, std::size_t k>
struct DuplicateDispatcher<Dispatcher, k, std::enable_if_t<k == 0>> {
  using type = TrivialDispatcher;
};

template <typename Dispatcher, std::size_t k>
struct DuplicateDispatcher<Dispatcher, k, std::enable_if_t<k != 0>> {
  using comb = typename DuplicateDispatcher<Dispatcher, k-1>::type;
  using type = typename comb::template set_next<Dispatcher>;
};

// -----------------------------------------------------------------------------
// Main definition of MultipleDispatcher.
...
struct MultipleDispatcher
  ...
  static constexpr std::size_t kOutputSize =
      entries::template apply<ResultLengthForEntry>
             ::template foldl<MetaAdd,
  ...
  using set_next = MultipleDispatcher<
      ...
      typename predicates::template append<
          typename NextDispatcher::predicates
              ...
              ::template apply<PredicateDelegatorShift>>>;
  ...
  using repeat = typename DuplicateDispatcher<
      MultipleDispatcher<EntryTL, PredicateTL, Transformer>, n>::type;
};
...
#endif  // QUICKSTEP_UTILITY_META_MULTIPLE_DISPATCHER_HPP_
